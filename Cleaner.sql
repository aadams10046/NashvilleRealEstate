...
--Create General Categories (Residential, Commercial, Non-Profit, Vacant, etc.), Count number of sales in each type, and calculate the average sale price
SELECT DISTINCT(LandUse) FROM Housing_Data ORDER BY 1 ASC;

SELECT
CASE
	WHEN LandUse IN ('SINGLE FAMILY','APARTMENT: LOW RISE (BUILT SINCE 1960)','CONDO', 'DUPLEX','MOBILE HOME' ,'QUADPLEX','RESIDENTIAL COMBO/MISC','RESIDENTIAL CONDO','TRIPLEX') THEN 'Residential'
	WHEN LandUse IN ('CONVENIENCE MARKET WITHOUT GAS','CONDOMINIUM OFC  OR OTHER COM CONDO','LIGHT MANUFACTURING','MORTUARY/CEMETERY','NIGHTCLUB/LOUNGE','OFFICE BLDG (ONE OR TWO STORIES)','ONE STORY GENERAL RETAIL STORE','RESTURANT/CAFETERIA','SMALL SERVICE SHOP','STRIP SHOPPING CENTER','TERMINAL/DISTRIBUTION WAREHOUSE') THEN 'Commercial'
	WHEN LandUse IN ('CHURCH','DORMITORY/BOARDING HOUSE','METRO OTHER THAN OFC, SCHOOL,HOSP, OR PARK','NON-PROFIT CHARITABLE SERVICE','PARSONAGE') THEN 'Non-Profit'
	WHEN LandUse IN ('FOREST','GREENBELT','GREENBELT/RES','GRRENBELT/RES','VACANT COMMERCIAL LAND','VACANT RES LAND','VACANT RESIDENTIAL LAND','VACANT RESIENTIAL LAND','VACANT RURAL LAND','VACANT ZONED MULTI FAMILY') THEN 'Land Only'
	ELSE 'Misc.'
END AS RealEstateCategory,
COUNT(DISTINCT(UniqueID)) AS NumberOfSales,
ROUND(AVG(SalePrice),2) AS AverageSalePrice
FROM Housing_Data
WHERE LandUse NOT NULL
GROUP BY RealEstateCategory
